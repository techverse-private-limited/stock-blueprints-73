...
export const FoodCategoryForm: React.FC<FoodCategoryFormProps> = ({ category, onSubmit, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error submitting food category:', error);
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 font-poppins">
      <div>
        <Label htmlFor="food_category_name" className="text-base font-medium font-poppins">
          Food Category Name *
        </Label>
        <Input
          id="food_category_name"
          value={name}
